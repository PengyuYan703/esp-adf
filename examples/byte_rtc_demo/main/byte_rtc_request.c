...
byte_rtc_req_result_t *byte_rtc_request(byte_rtc_req_config_t *config)
{
    ...
    // byte_rtc_req_result_t *req_result = impl_malloc_fn(sizeof(byte_rtc_req_result_t));
    // assert(req_result);

    const char *req_body = str_pkg_to_cjson_fmt(config);
    ...
    byte_rtc_req_result_t *req_result = cjson_unpkg_2_str_fmt(rsp_data.rsp_buffer);
