...
static const char *TAG = "bmi270_i2c_common";
...
static i2c_bus_handle_t intf_conf;
...
{
    ESP_LOGI(TAG, "bmi2_i2c_read=================");
    i2c_bus_handle_t intf_info = (i2c_bus_handle_t)intf_ptr;
    // return i2c_bus_read_bytes(intf_info, reg_addr, (uint16_t)len, reg_data);
    esp_err_t res = i2c_bus_read_bytes(intf_info, BMI270_CHIP_ID, &reg_addr, 1, reg_data, (uint16_t)len);

    if (res == ESP_OK) {
        ESP_LOGE(TAG, "bmi2_i2c_read ok");
    }else {
        ESP_LOGE(TAG, "bmi2_i2c_read fail");
    }
    return res;
    
    // return i2c_master_write_read_device(I2C_NUM_0, BMI270_CHIP_ID, &reg_addr, 1, reg_data, len, 1);
}
...
{
    ESP_LOGI(TAG, "bmi2_i2c_write=================");
    i2c_bus_handle_t intf_info = (i2c_bus_handle_t)intf_ptr;
    // return i2c_bus_write_bytes(intf_info, reg_addr, len, reg_data);
    return i2c_bus_write_bytes(intf_info, BMI270_CHIP_ID, &reg_addr, 1, (uint8_t *)reg_data, (int)len);
    
    // return i2c_master_write_to_device(I2C_NUM_0, BMI270_CHIP_ID, reg_data, len, 1);
}
...
int8_t bmi2_interface_init(struct bmi2_dev *bmi, uint8_t intf, uint8_t dev_addr, i2c_bus_handle_t bus_inst)
{
    ...
    if (bmi != NULL) {
        ...
        if (BMI2_OK == result) {
            ...
            // i2c_bus_handle_t i2c_device_handle = i2c_bus_device_create(bus_inst, dev_addr, 0);
            ...
            // };
            // get_i2c_pins(I2C_NUM_0, &i2c_conf);
            // ESP_LOGE(TAG, "====%d ========%d", i2c_conf.sda_io_num, i2c_conf.scl_io_num);
            // i2c_bus_handle_t i2c_device_handle = i2c_bus_create(I2C_NUM_0, &i2c_conf);
            if (NULL == bus_inst) {
                ESP_LOGE("BMI2", "i2c_bus_device_create failed");
                rslt = BMI2_E_NULL_PTR;
            }
